...
func FuzzStringMap8(f *testing.F) {
	f.Add(uint8(1), 14, 50)
	f.Add(uint8(2), 1, 1)
	f.Add(uint8(2), 14, 14)
	f.Add(uint8(2), 14, 15)
	...
	f.Fuzz(func(t *testing.T, keySz uint8, init, count int) {
		...
		fuzzTestStringMap8(t, uint32(keySz), uint32(init), uint32(count))
	})
}

func fuzzTestStringMap8(t *testing.T, keySz, init, count uint32) {
	const limit = 1024 * 1024
	if count > limit || init > limit {
		t.Skip()
	}
	m := NewMap8[string, int](init)
	if count == 0 {
		return
	}
	...
	keys := genStringData8(int(keySz), int(count))
...
func setConstSeed8[K comparable, V any](m *Map8[K, V], seed uintptr) {
	h := (*hasher8)((unsafe.Pointer)(&m.hash))
	h.seed = seed
}
